...
export default function Dashboard() {
  ...
  // Svi izvještaji sortirani po datumu (najnoviji prvi)
  ...
  function onDeleteAll() {
    if (!confirm("Obrisati SVE izvještaje ovog naloga? Ova akcija je nepovratna.")) {
      return;
    }
  ...
  function onRowEdit(report: LabReport, idx: number, row: LabRow) {
    const nv = prompt(
      `Nova vrijednost za "${row.Analit}" (trenutno: ${row.Vrijednost ?? ""})`,
    ...
    // jednostavna provjera statusa u odnosu na referentni opseg
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
      ...
      {/* LabGuard AI chat blok – dobija sve izvještaje (bot sam flatten-uje u redove) */}
      <LabGuardChat reports={reports} />
    </div>
  );
}
