...
const Navigation = () => {
  ...
  const visibleNavItems = navItems.filter(
    (item) => session || item.path !== "/moji-nalazi"
  );

  const getDefaultChecked = (itemPath: string, index: number) => {
    if (location.pathname === itemPath) return true;
    // ako si na nekoj nepoznatoj ruti, prva kartica je checked
    if (!navItems.some((i) => i.path === location.pathname) && index === 0) {
      return true;
    }
    return false;
  };
  ...
  return (
    <nav className="sticky top-0 z-50 border-b border-transparent bg-gradient-to-b from-white/70 to-white/40 backdrop-blur-xl">
      {/* SVG filter za liquid glass efekat – isti kao u originalnom primjeru */}
      <svg
              ...
              {visibleNavItems.map((item, index) => (
                <label key={item.path} className="glass-nav-option">
                  <input
                    ...
                    type="radio"
                    name="nav-main"
                    defaultChecked={getDefaultChecked(item.path, index)}
